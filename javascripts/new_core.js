if(typeof(pidCrypt) != 'undefined'){
  pidCrypt.AES = function(env) {
    ...
    pidCrypt.AES.prototype.init = function (key) {
      ...
      if ((key.length == 4) || (key.length == 8)) {
          pidCrypt.AES.wordsToBytes(key, this._key);
      }
      else
          pidCrypt.AES.bytesToWords(key, this._key);
    ...
    pidCrypt.AES.prototype.scheduleEncrypt = function () {
      this._decryptScheduled = false;
      this._w = [];
      var key = [];
      if ((this._key.length == 16) || (this._key.length == 32)) pidCrypt.AES.bytesToWords(this._key, key);
      else key = this._key;
      var klen = key.length;
      var j = 0;
    ...
    pidCrypt.AES.prototype.scheduleDecrypt = function() {
    ...
    */
    pidCrypt.AES.prototype.encryptBlock = function(block, ciphertext, decrypt) {
    ...
    pidCrypt.AES.prototype.decryptBlock = function(block, plaintext) {
        if (!this._decryptScheduled) this.scheduleDecrypt();

      this.encryptBlock(block, plaintext, true);
    };

    // block and plaintext are bytes arrays of 32 bytes length
    pidCrypt.AES.prototype.encrypt = function(plaintext,block) {
      alert('sasdasd');
      pidCrypt.AES.bytesToWords(plaintext.slice(), plaintext);
      this.encryptBlock(plaintext, block);
      pidCrypt.AES.wordsToBytes(block.slice(), block);
    };


   // block and plaintext are bytes arrays of 32 bytes length
    pidCrypt.AES.prototype.decrypt = function(block,plaintext) {
      pidCrypt.AES.bytesToWords(block.slice(), block);
      this.decryptBlock(block, plaintext);
      pidCrypt.AES.wordsToBytes(plaintext.slice(), plaintext);
    };
    ...
    pidCrypt.AES._hexall = function(nn) {
      var out = "",i;
      for (i=0;i<nn.length;i++) {
        if (i%4 == 0) out+= "<br/>\n";
        else if (i) out += " ";
        out += pidCrypt.AES._hex(nn[i]);
      }
      return out;
    }

    pidCrypt.AES.bytesToAscii = function(bytes) {
      var ascii = "";
    ...
    pidCrypt.AES.asciiToBytes = function(ascii, bytes) {
      var len = ascii.length;
      for (var i=0; i < len; i++)
        bytes[i] = ascii.charCodeAt(i);
    };

    pidCrypt.AES.wordsToBytes = function(words, bytes) {
      var bitmask = 1;
    ...
    pidCrypt.AES.bytesToWords = function(bytes, words) {
    ...
    pidCrypt.AES.prototype._precompute = function() {
